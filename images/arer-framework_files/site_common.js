var ingentaCMSApp = ingentaCMSApp || {
  ...
  /**
    ...
    displayElipsisDescription: function() {
        ...
        descNode.each(function() {
            //for each item remove the read more link if it exists this is needed for serach page as loaded via ajax
            $(this).parent().find('.readmore').remove();

            //if the text is longer than the container then add a read more link
            if ($(this).height() < $(this).prop('scrollHeight')) {
                const readMoreURL = $(this).closest('.js-browse-item').find('.js-articleTitle, .js-title').children('a').attr('href'), 
                    readMoreURLAlt = function() { 
                                    return $(this).closest('.js-browse-item').find('.js-title').children('a').attr('href');
                    },
                ...
                $(this).after($("<a href='" + (readMoreURL ? readMoreURL : readMoreURLAlt()) + "' class='readmore' aria-labelledby="+ (readMoreID ? readMoreID : readMoreIDAlt()) +">Read More</a>"));
            } else {
                //if the text is not longer than the container then override the fixed height class
                $(this).addClass('remove-fixed-height');
            }
        });
    },
...
(function(){
  ...
  $('main').on('click', ftDownloadLinkElemnts,  function (event) {
    ...
    $("#html_fulltext, #dataandmedia").on("click", "a:not(.js-toggle-table)", function(event){
        //matches the character "#" literally first char.
        // [tf] matches either "t" or "f" 2nd char.
        // \d+ matches one or more digits after that.
        const regExMatchTableFigureLink = /^#[tf]\d+$/;
        const divID = $(this).attr('href');
        if(divID.startsWith("#") && divID !== '#' && !regExMatchTableFigureLink.test(divID)) {
            const divIDElement = $(divID)[0];
            const sidebarElement = $('#sidebar_right');
            if (divIDElement) {
             const divIDElementTop = divIDElement.getBoundingClientRect().top;
             const sidebarElementScrollTop = sidebarElement.scrollTop();
             const offsetTop = sidebarElementScrollTop + divIDElementTop;
              event.stopImmediatePropagation();
                 $('#sidebar_right').animate({ scrollTop: offsetTop }, 'slow');
                 divIDElement.setAttribute('tabindex', '-1');
                 divIDElement.focus();
            }
        }
     });
