...
$(document).on('keydown', function(event) {
    if (event.key === "Escape" || event.keyCode === 27) {
        ...
        //ensure focus returns to the dropdown toggle button/link aka opener
        $('.dropdown.open').children('a').focus();
        $('.dropdown').removeClass('open');
    }
});

$(document).on('click','.menu-sections.disabled', function(e){
    //prevent the default action of the link to avoid the page from scrolling to the top    
    e.preventDefault();
});
  ...
  $(document).on("click", ".js-seeMoreFacetsLink a", function(e) {
      ...
      /* ajax section - view more */
              var  url = parentContainer.data('ahahurl'),
                  ...
                  console.log('{"' + decodeURI(searchurl.substring(searchurl.indexOf('search?')+7)).replace(/"/g, '\\"').replace(/&/g, '","').replace(/=/g,'":"') + '")');
...
$(window).on("scroll", function() {
      ...
      var articleNavigationBarPosToTopViewport = (articleNavigationBarPosToTopDocument - scroll);
        ...
        if (articleNavigationBar.length > 0) {
            ...
            if (articleNavigationBarPosToTopViewport < 4){
...
$(document).on("click", ".js-viewFootnotePopup",function(e) {
       e.preventDefault();
       var self = $(this);
       var signup = $("#viewFootnotePopup");
       signup.modal('show');
  ...
  $(document).click(function (event) {
    ...
    // start content collections full description show hide
    const collectionDesc = $('.collection__description--show-full');
    const collectionDescShowFull = $('.js-collection__description--show-full');
    const collectionDescHideLastPart = $('.js-collection__description--hide-last-part');

    if(collectionDescShowFull != null || collectionDescHideLastPart != null){
        const numberOfLineElement = document.querySelector('.collection__description--number-of-line');
        const numberOfLine = numberOfLineElement != null ? numberOfLineElement.textContent : 0;
        const divElement = document.querySelector('.collection__description--show-full');

        const divHeight = divElement != null ? Number(window.getComputedStyle(divElement).getPropertyValue('height').slice(0, -2)) : 0;
        const lineHeight = divElement != null ? Number(window.getComputedStyle(divElement).getPropertyValue('line-height').slice(0, -2)) : 0;
        const maxHeight = numberOfLine * lineHeight;
        const content = divElement != null ? divElement.innerText : null;

        const showHidBtnDivElement = $('.collection__description--show-hide');
        if(content != null && divHeight>=(maxHeight+lineHeight)){
            collectionDesc.css('max-height', maxHeight);
            collectionDescHideLastPart.css('display','none');

            collectionDescShowFull.click(function(){
                collectionDescShowFull.css('display', 'none');
                collectionDescHideLastPart.css('display','block');
                collectionDesc.css('max-height', 'none');
            });

            collectionDescHideLastPart.click(function(){
                collectionDescHideLastPart.css('display','none');
                collectionDescShowFull.css('display', 'block');
                collectionDesc.css('max-height', maxHeight);
            });
        } else {
            showHidBtnDivElement.css('display', 'none');
        }
    }
    // end content collections full description show hide
